...
std::string VulnerablePenguinEncrypt(const std::string& my_string, const std::string& key_string) {
    std::array<uint8_t, 16> key{
    ...
    std::string input_hex_string{StringToHexString(my_string) + secret_hex_string};
...
int main() {
    ...
    std::string block_size_test{VulnerablePenguinEncrypt(std::string(128, 'A'), key_string)};

    bool block_size_found = false;
    size_t probable_block_size;
    for (size_t block_size = 4; (block_size <= 64) && !block_size_found; block_size++) {
        std::string ss_0 = block_size_test.substr(0, block_size);
        std::string ss_1 = block_size_test.substr(block_size, block_size);
    ...
    std::cout << "probable ECB, block size: " << probable_block_size << std::endl;

    size_t secret_length = VulnerablePenguinEncrypt("", key_string).length();
    ...
    for (size_t i = 0; i < secret_length; i++) {
        std::string penguin_input_to_match{VulnerablePenguinEncrypt(known_padding_input.substr(0, (probable_block_size - (i % probable_block_size) - 1)), key_string).substr(16 * static_cast<size_t>(i / 16), 16)};

        for (unsigned c = 0; c < 256; c++) {
            known_padding_input[known_padding_input.length() - 1] = static_cast<char>(c);

            if (penguin_input_to_match == VulnerablePenguinEncrypt(known_padding_input, key_string).substr(0, 16)) {
